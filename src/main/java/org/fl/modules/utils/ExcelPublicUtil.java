...
import org.fl.modules.excel.poi.annotation.ExcelEntity;
import org.fl.modules.excel.poi.annotation.ExcelIgnore;
	...
	public static boolean isNotUserExcelUserThis(Field field, String targetId) {
		...
		if (field.getAnnotation(ExcelIgnore.class) != null) {
			boo = true;
				...
				&& isUseInThis(
				field.getAnnotation(ExcelCollection.class).exportName(),
				targetId)) {
			boo = false;
			...
			boo = false;
				...
				&& isUseInThis(
				field.getAnnotation(ExcelEntity.class).exportName(),
				targetId)) {
			boo = false;
		}
		return boo;
		...
		try {
			...
			for (Field field : fields) {
				...
				if (isCollection(field.getType())) {
					...
					setMethod.invoke(obj, ExcelPublicUtil.class.getClassLoader()
							.loadClass(collection.type()).newInstance());
